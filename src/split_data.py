    ...
    for workshop_file in workshop_files:
        ...
        try:
            ...
            if not workshop_match:
                print(
                    f"  Warning: Could not extract workshop number from {workshop_file.name}"
                )
                continue
            ...
            for group_name, group_data in df.groupby("name"):
                ...
                group_match = re.search(r"group\s*(\d+)", str(group_name).lower())
                if not group_match:
                    print(
                        f"    Warning: Could not extract group number from '{group_name}'"
                    )
                    continue
                ...
                # Drop the 'z' column if it exists
                if "z" in group_data.columns:
                    group_data = group_data.drop(columns=["z"])
                # Save the group data
