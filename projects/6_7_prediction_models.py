...
def extract_features_for_prediction(df, stations):
    ...
    for group_name in sorted(df['name'].unique()):
        ...
        if len(station_sequence) >= 2:
            total_time = (station_sequence[-1]['exit_time'] - station_sequence[0]['entry_time']).total_seconds()
...
def train_production_model(production_df):
    ...
    # Prepare features and target
    X = production_df[['num_stations', 'avg_dwell_time', 'first_station', 'last_station']]
...
def plot_prediction_results(results, title, ylabel):
    ...
    ax.scatter(results['y_test'], results['y_pred'], alpha=0.6, s=100, edgecolors='black')
...
for workshop_file in WORKSHOP_FILES:
    ...
    if trans_results:
        ...
        # Plot
        fig = plot_prediction_results(trans_results, 
                                      f'{workshop_name} - Transition Time Prediction',
                                      'Time (seconds)')
        plot_path = os.path.join(OUTPUT_FOLDER, f'{workshop_name}_transition_prediction.png')
    ...
    if prod_results:
        print(f"  MAE: {prod_results['mae']:.2f} seconds ({prod_results['mae']/60:.2f} minutes)")
        print(f"  RMSE: {prod_results['rmse']:.2f} seconds ({prod_results['rmse']/60:.2f} minutes)")
        print(f"  R² Score: {prod_results['r2']:.3f}")
        
        # Plot
        fig = plot_prediction_results(prod_results,
                                      f'{workshop_name} - Production Time Prediction',
                                      'Time (seconds)')
        plot_path = os.path.join(OUTPUT_FOLDER, f'{workshop_name}_production_prediction.png')
        fig.savefig(plot_path, dpi=150, bbox_inches='tight')
        print(f"  ✓ Saved plot: {plot_path}")
        plt.close(fig)
