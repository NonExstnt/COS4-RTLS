    ...
    for group_name in sorted(df['name'].unique()):
        ...
        for idx, row in group_data.iterrows():
            ...
            if station is not None and station < max_station_reached:
                station = current_station  # Treat as sensor drift, stay at current
        ...
        for station_id in range(1, len(stations) + 1):
            station_times = [v['dwell_seconds'] for v in station_visits if v['station'] == station_id]
...
def create_dwell_comparison_chart(dwell_df, workshop_name):
    ...
    pivot_data = dwell_df.pivot(index='group', columns='station', values='dwell_time_minutes')
    ...
    pivot_data.plot(kind='bar', stacked=True, ax=ax, 
                   colormap='tab10', width=0.7, edgecolor='black')
    
    ax.set_xlabel('Group', fontsize=12, fontweight='bold')
    ax.set_ylabel('Dwell Time (minutes)', fontsize=12, fontweight='bold')
    ax.set_title(f'{workshop_name} - Station Dwell Time Comparison', 
